...
#define CANBUS_SHIELD_V1   // uncomment if this shield

//#ifdef CANBUS_SHIELD_V1 
#define CRYSTAL MCP_16MHZ
//else
//#define CRYSTAL MCP_8MHZ  // standard 8MHz crystal
//#endif
...
{
    Serial.println("UNO Canbus monitor V1.2");
    while (CAN_OK != CAN.begin(MCP_ANY, CAN_125KBPS, CRYSTAL))              // init can bus : baudrate = 500k
    {
...
void loop()
{
  ...
  if(Serial.available())
         ...
         switch(result[0])     // result[0] contain the command
           {
            case CANBUSspeed: Serial.print("canbus speed "); Serial.println(result[1]);  
                if(result[1]==125) CAN.begin(MCP_ANY, CAN_125KBPS, CRYSTAL);
                if(result[1]==250) CAN.begin(MCP_ANY, CAN_250KBPS, CRYSTAL);
                if(result[1]==500) CAN.begin(MCP_ANY, CAN_500KBPS, CRYSTAL);
                CAN.setMode(MCP_NORMAL);   // Change to normal mode to allow messages to be transmitted                 break; 
             case CANBUStransmit:     // transmit RTR or data frame
               ...
               Serial.println();
               //  byte sendMsgBuf(unsigned long id, byte ext, byte len, const byte *buf);               // send buf
               byte sndStat = CAN.sendMsgBuf(result[1], result[2], 8, stmp); 
    ...
    {
        unsigned long int canId=0;// = CAN.getCanId();
        ...
        if((canId & 0x80000000) == 0x80000000) strcat(text,">Rx EXT "); else strcat(text,">Rx STD "); 
        sprintf(&text[strlen(text)],"ID %08lx ", canId);
        Serial.print(text);
        // check RTR or data frame
        if((canId & 0x40000000) == 0x40000000) Serial.print(" RTR ");   
        else
        {
           Serial.print("data ");
           for(int i = 0; i<8; i++)    // print the data
